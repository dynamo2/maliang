...
function editPropertiesTable(props){
	var tObj = $("<form  class='form-horizontal' id='editPropertiesTable' />");//newEditTable("editPropertiesTable");
	...
	for(x in props){
		...
		var field = props[x];
		var trObj = $("<div class='form-group'>");
		
		trObj.append(newLabel(field.name)); //add label td
		trObj.append(newEditDiv(field)); // add value td
...
function newLabel(key){
	var obj = $("<label for='inputEmail3' class='col-sm-1 control-label' />");
	obj.text(metadataModel.getLabel(key));
	return obj;
}

function newEditDiv(field){
	var tdObj = $("<div class='col-sm-6' />");
...
function addEditPanelTab(id,tabName,table){
	$("#panelList").append($("<li id='"+id+"List' role='presentation'><a href='#"+id+"' aria-controls='home' role='tab' data-toggle='tab'>"+tabName+"</a></li>"));
	$("#editTabContent").append($("<div id='"+id+"' role='tabpanel' class='tab-pane' />").append(table));  
}

function newEditTable(tableId){
	return $("<table class='table table-striped' cellpadding='0' cellspacing='1' id='"+tableId+"' />");
}
