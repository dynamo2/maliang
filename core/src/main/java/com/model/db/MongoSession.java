...
public class MongoSession extends AbstractMongoSession {
    ...
    public Object get(ObjectId id) {
        try {
            ...
            Document document = this.getCollection().find(query).first();
    ...
    public Object find(Object obj) {
        try {
        	//Bson query = Filters.eq("_id", BsonUtil.toBson(obj).get("_id"));
        	Bson query = BsonUtil.toBson(obj);
    ...
    public List<Object> finds() {
        ...
        System.out.println("--------- getCollectionName " + this.getCollection().getNamespace().getCollectionName());
    ...
    public List<Object> query(Bson bson, Bson sort,Pager page) {
    	long totalRow = this.getCollection().count(bson);
    	page.setTotalRow((int)totalRow);
    	
    	int limit = page.getPageSize();
    	int skip = (page.getCurPage()-1)*page.getPageSize();
    	
        FindIterable<Document> doIterable = this.getCollection().find(bson).sort(sort);
        if (limit > 0) {
            doIterable = doIterable.limit(limit);
        }
        if (skip > 0) {
