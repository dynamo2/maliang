...
//		System.out.println(dao.dbColl.find(dao.getObjectId("56f9fac750776d16e7891bb5")).toArray());
		...
		wf.setResponse("{html_template:'<div id='edit_form'></div>',"
				...
				+ "{name:'product.name',label:'鍚嶇О',type:'text'},"
				+ "{name:'product.brand',label:'鍝佺墝',type:'select',options:each(brands){{key:this.id,label:this.name}}},"
				+ "{name:'product.price',label:'浠锋牸',type:'number'}],"
				+ "ul-list:{header:[{name:'name',label:'鍚嶇О'},{name:'brand',label:'鍝佺墝'},{name:'price',label:'浠锋牸'},{name:'picture',label:'鍥剧墖'},{name:'operator',label:'鎿嶄綔'}],"
				...
				+ "operator:[{type:'a',href:'/edit.htm?id='+this.id,text:'缂栬緫'},{type:'a',href:'/delete.htm?id='+this.id,text:'鍒犻櫎'}]}}}"
				+ "}}]}");
		wf.setStep(1);
		wfs.add(wf);
